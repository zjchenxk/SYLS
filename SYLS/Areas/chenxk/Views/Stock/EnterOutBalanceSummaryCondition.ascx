<%@ Control Language="C#" Inherits="System.Web.Mvc.ViewUserControl<SYLS.Areas.chenxk.ViewModels.EnterOutBalanceSummaryConditionViewModel>" %>
<% using (Html.BeginForm("", "", FormMethod.Post, new { id = "formEnterOutBalanceSummaryCondition" }))
   { %>
<table>
    <tr>
        <td>
...
<script type="text/javascript">
    ...
    $(document).ready(function () {
        //设置起始时间
        $("#formEnterOutBalanceSummaryCondition #StartTime").datepicker($.datepicker.regional["zh-CN"]);

        //设置截止时间
        $("#formEnterOutBalanceSummaryCondition #EndTime").datepicker($.datepicker.regional["zh-CN"]);

        //设置客户名称自动完成
        $("#formEnterOutBalanceSummaryCondition #CustomerName").autocomplete({
            source: "/Autocomplete/LoadCustomerNames/",
            minLength: 1
        });

        //设置货物编号自动完成
        $("#formEnterOutBalanceSummaryCondition #GoodsNo").autocomplete({
