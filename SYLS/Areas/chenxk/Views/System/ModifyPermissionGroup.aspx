...
<asp:Content ID="Content1" ContentPlaceHolderID="TitleContent" runat="server">
    <%: InnoSoft.LS.Resources.Titles.ModifyPermissionGroup %>
</asp:Content>
...
<asp:Content ID="Content3" ContentPlaceHolderID="WorkspaceContent" runat="server">
    <% using (Html.BeginForm("ModifyPermissionGroup", "System", FormMethod.Post, new { id = "form1" }))
       { %>
    <fieldset>
        <legend>
        ...
        $(document).ready(function () {
            ...
            $("#grid1").jqGrid({
                ...
                gridComplete: function () {
                    //加载当前组权限数据
                    $.get("/chenxk/System/LoadGroupPermissions", { nGroupId: $("#Id").val().trim() }, function (ret) {
                        $.each(ret, function (i, func) {
                            var ind = $("#grid1").jqGrid("getInd", func.FuncId);
                            if (ind > 0) {
                                $("#grid1").jqGrid("setRowData", func.FuncId, { AllowOpen: func.AllowOpen ? "O" : "X", AllowNew: func.AllowNew ? "O" : "X", AllowModify: func.AllowModify ? "O" : "X", AllowDelete: func.AllowDelete ? "O" : "X", AllowCancel: func.AllowCancel ? "O" : "X", AllowDetail: func.AllowDetail ? "O" : "X", AllowSearch: func.AllowSearch ? "O" : "X", AllowSubmit: func.AllowSubmit ? "O" : "X", AllowApprove: func.AllowApprove ? "O" : "X", AllowDispatch: func.AllowDispatch ? "O" : "X", AllowExport: func.AllowExport ? "O" : "X", AllowImport: func.AllowImport ? "O" : "X", AllowPrint: func.AllowPrint ? "O" : "X" });
                            }
                        });
            ...
            $('#form1').submit(function () {
                if ($(this).valid()) {
                    ...
                    //生成JSON数据
                    var o = { "Id": $("#Id").val().trim(), "Name": $("#Name").val().trim(), "Remark": $("#Remark").val().trim(), "Functions": functions };
