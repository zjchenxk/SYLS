<%@ Page Title="" Language="C#" MasterPageFile="~/Views/Shared/Workspace.master"
    Inherits="System.Web.Mvc.ViewPage<SYLS.Areas.chenxk.ViewModels.PriceApproveFlowStepViewModel>" %>

<asp:Content ID="Content1" ContentPlaceHolderID="TitleContent" runat="server">
    <%: InnoSoft.LS.Resources.Titles.ModifyPriceApproveStep %>
</asp:Content>
<asp:Content ID="Content2" ContentPlaceHolderID="MenuContent" runat="server">
    <ul id="menu">
        <li><a href="/chenxk/Flow/SetPriceApproveFlow/" onclick="javascript:location.replace(this.href);return false;">
...
<asp:Content ID="Content3" ContentPlaceHolderID="WorkspaceContent" runat="server">
    <% using (Html.BeginForm("ModifyPriceApproveStep", "Flow", FormMethod.Post, new { id = "form1" }))
       { %>
    <fieldset>
        <legend>
    ...
    <div id="dialogConditions" title="">
        <% Html.RenderPartial("PriceApproveFlowStepCondition", Model.Conditions[0]); %>
    </div>
    <script type="text/javascript">
        ...
        function WorkspaceContent() {
            ...
            this.newCondition = function () {
                ...
                //初始化控件数据
                $("#formPriceApproveFlowStepCondition #FieldValue").val("");
            ...
            this.modifyCondition = function () {
                ...
                //初始化控件数据
                $("#formPriceApproveFlowStepCondition #FieldName").val(data.FieldName);
                $("#formPriceApproveFlowStepCondition #CompareOperator").val(data.CompareOperator);
                $("#formPriceApproveFlowStepCondition #FieldValue").val(data.FieldValue);
        ...
        $(document).ready(function () {
            //创建条件表格
            $("#grid1").jqGrid({
                url: "/chenxk/Flow/LoadPriceApproveFlowStepConditionsGrid/" + encodeURI($("#form1 #Id").val().trim()),
                datatype: "json",
                mtype: "GET",
                colNames: ["<%: InnoSoft.LS.Resources.Labels.ConditionId %>", "<%: InnoSoft.LS.Resources.Labels.ConditionNum %>", "<%: InnoSoft.LS.Resources.Labels.FieldName %>", "<%: InnoSoft.LS.Resources.Labels.CompareOperator %>", "<%: InnoSoft.LS.Resources.Labels.FieldValue %>"],
                colModel: [
            ...
            $("#dialogConditions").dialog({
                ...
                buttons: {
                    "<%: InnoSoft.LS.Resources.Buttons.Ok %>": function () {
                        if ($("#formPriceApproveFlowStepCondition").valid()) {
                            //debugger;
                            //新增条件信息
                            if (objMain.objWorkspace.objContent.isNewCondition) {
                                var rowcount = $("#grid1").jqGrid("getGridParam", "records");
                                if (rowcount == 0) {
                                    $("#grid1").jqGrid("addRowData", 1, { ConditionNum: 1, FieldName: $("#formPriceApproveFlowStepCondition #FieldName").val().trim(), CompareOperator: $("#formPriceApproveFlowStepCondition #CompareOperator").val().trim(), FieldValue: $("#formPriceApproveFlowStepCondition #FieldValue").val().trim() });
                                }
                                else {
                                    var rowids = $("#grid1").jqGrid("getDataIDs");
                                    var rowidLast = rowids[rowcount - 1];

                                    $("#grid1").jqGrid("addRowData", parseFloat(rowidLast) + 1, { ConditionNum: rowcount + 1, FieldName: $("#formPriceApproveFlowStepCondition #FieldName").val().trim(), CompareOperator: $("#formPriceApproveFlowStepCondition #CompareOperator").val().trim(), FieldValue: $("#formPriceApproveFlowStepCondition #FieldValue").val().trim() });
                                }
                            }
                            //修改条件信息
                            else {
                                $("#grid1").jqGrid("setRowData", objMain.objWorkspace.objContent.selectedConditionId, { FieldName: $("#formPriceApproveFlowStepCondition #FieldName").val().trim(), CompareOperator: $("#formPriceApproveFlowStepCondition #CompareOperator").val().trim(), FieldValue: $("#formPriceApproveFlowStepCondition #FieldValue").val().trim() });
                            }
                            $(this).dialog("close");
                        }
                    },
            ...
            $("#form1").submit(function () {
                if ($(this).valid()) {
                    ...
                    //生成JSON数据
                    var o = { "Id": $("#form1 #Id").val().trim(), "StepNum": $("#form1 #StepNum").val().trim(), "StepName": $("#form1 #StepName").val().trim(), "DisposerId": $("#form1 #DisposerId").val().trim(), "ConditionExpression": $("#form1 #ConditionExpression").val().trim(), "Conditions": conditions };
                    ...
                    $.ajax({
                        ...
                        success: function (ret) {
                            ...
                            if (ret != null && ret != "") {
                                ...
                                location.replace("/chenxk/Flow/SetPriceApproveFlow/");
                            }
                        },
