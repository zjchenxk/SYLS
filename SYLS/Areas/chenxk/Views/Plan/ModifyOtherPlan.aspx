<%@ Page Title="" Language="C#" MasterPageFile="~/Views/Shared/Workspace.master"
    Inherits="System.Web.Mvc.ViewPage<SYLS.Areas.chenxk.ViewModels.OtherPlanViewModel>" %>

<asp:Content ID="Content1" ContentPlaceHolderID="TitleContent" runat="server">
    <%: InnoSoft.LS.Resources.Titles.ModifyOtherPlan %>
</asp:Content>
...
<asp:Content ID="Content3" ContentPlaceHolderID="WorkspaceContent" runat="server">
    <% using (Html.BeginForm("ModifyOtherPlan", "Plan", FormMethod.Post, new { id = "form1" }))
       { %>
    <fieldset>
        <legend>
    ...
    <div id="dialogGoods" title="">
        <% Html.RenderPartial("OtherPlanGoods", Model.Goods[0]); %>
    </div>
    <script type="text/javascript">
        ...
        function WorkspaceContent() {
            ...
            this.newGoods = function () {
                ...
                $("#formOtherPlanGoods #Packages").val("");
                $("#formOtherPlanGoods #Tunnages").val("");
            ...
            this.modifyGoods = function () {
                ...
                //初始化控件数据
                $("#formOtherPlanGoods #GoodsId").val(data.GoodsId);
                $("#formOtherPlanGoods #GoodsNo").val(data.GoodsNo);
                $("#formOtherPlanGoods #GoodsName").val(data.GoodsName);
                $("#formOtherPlanGoods #SpecModel").val(data.SpecModel);
                $("#formOtherPlanGoods #Packages").val(data.Packages);
                $("#formOtherPlanGoods #Tunnages").val(data.Tunnages);
        ...
        $(document).ready(function () {
            ...
            //创建货物表格
            $("#grid1").jqGrid({
                url: "/chenxk/Plan/LoadOtherPlanGoodsGrid/" + encodeURI($("#form1 #Id").val().trim()),
                datatype: "json",
                mtype: "GET",
                colNames: ["<%: InnoSoft.LS.Resources.Labels.GoodsId %>", "<%: InnoSoft.LS.Resources.Labels.GoodsNo %>", "<%: InnoSoft.LS.Resources.Labels.GoodsName %>", "<%: InnoSoft.LS.Resources.Labels.Specification %>", "<%: InnoSoft.LS.Resources.Labels.Pieces %>", "<%: InnoSoft.LS.Resources.Labels.Tunnages %>"],
                colModel: [
            ...
            $("#dialogGoods").dialog({
                ...
                buttons: {
                    "<%: InnoSoft.LS.Resources.Buttons.Ok %>": function () {
                        if ($("#formOtherPlanGoods").valid()) {
                            //新增货物信息
                            if (objMain.objWorkspace.objContent.isNewGoods) {
                                var rowcount = $("#grid1").jqGrid("getGridParam", "records");
                                if (rowcount == 0) {
                                    $("#grid1").jqGrid("addRowData", 1, { GoodsId: $("#formOtherPlanGoods #GoodsId").val().trim(), GoodsNo: $("#formOtherPlanGoods #GoodsNo").val().trim(), GoodsName: $("#formOtherPlanGoods #GoodsName").val().trim(), SpecModel: $("#formOtherPlanGoods #SpecModel").val().trim(), Packages: $("#formOtherPlanGoods #Packages").val().trim(), Tunnages: $("#formOtherPlanGoods #Tunnages").val().trim() });

                                    //重置合计数
                                    $("#grid1").jqGrid("footerData", "set", { GoodsNo: "<%: InnoSoft.LS.Resources.Labels.Total %>", Packages: $("#formOtherPlanGoods #Packages").val().trim(), Tunnages: $("#formOtherPlanGoods #Tunnages").val().trim() });
                                }
                                else {
                                    //检查货物是否重复并累计合计数
                                    var totalPackages = 0;
                                    ...
                                    for (var i = 0; i < rows.length; i++) {
                                        var row = rows[i];
                                        if (row.GoodsNo == $("#formOtherPlanGoods #GoodsNo").val().trim()) {
                                    ...
                                    $("#grid1").jqGrid("addRowData", parseFloat(rowidLast) + 1, { GoodsId: $("#formOtherPlanGoods #GoodsId").val().trim(), GoodsNo: $("#formOtherPlanGoods #GoodsNo").val().trim(), GoodsName: $("#formOtherPlanGoods #GoodsName").val().trim(), SpecModel: $("#formOtherPlanGoods #SpecModel").val().trim(), Packages: $("#formOtherPlanGoods #Packages").val().trim(), Tunnages: $("#formOtherPlanGoods #Tunnages").val().trim() });

                                    //重置合计数
                                    $("#grid1").jqGrid("footerData", "set", { GoodsNo: "<%: InnoSoft.LS.Resources.Labels.Total %>", Packages: totalPackages + parseInt($("#formOtherPlanGoods #Packages").val().trim()), Tunnages: add(totalTunnages, parseFloat($("#formOtherPlanGoods #Tunnages").val().trim())) });
                                }
                            }
                            //修改货物信息
                            else {
                                ...
                                for (var i = 0; i < rows.length; i++) {
                                    var rowid = rowids[i];
                                    var row = rows[i];
                                    if (row.GoodsNo == $("#formOtherPlanGoods #GoodsNo").val().trim() && rowid != objMain.objWorkspace.objContent.selectedGoodsId) {
                                        alert("<%: InnoSoft.LS.Resources.Strings.GoodsIsExists %>");
                                        return;
                                    }
                                ...
                                $("#grid1").jqGrid("setRowData", objMain.objWorkspace.objContent.selectedGoodsId, { GoodsId: $("#formOtherPlanGoods #GoodsId").val().trim(), GoodsNo: $("#formOtherPlanGoods #GoodsNo").val().trim(), GoodsName: $("#formOtherPlanGoods #GoodsName").val().trim(), SpecModel: $("#formOtherPlanGoods #SpecModel").val().trim(), Packages: $("#formOtherPlanGoods #Packages").val().trim(), Tunnages: $("#formOtherPlanGoods #Tunnages").val().trim() });
